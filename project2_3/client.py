...
class Client(BaseClient):
    ...
    def merkelStore(self, ptr_node3, key2_sym_priv, key2_mac_priv, node3_value):
        ...
        if len(node3_value) <= BLOCK_SIZE:
            ...
            ptr_left_child = self.crypto.get_random_bytes(64)
            ptr_right_child = self.crypto.get_random_bytes(64)
            root_value = "[HASH]" + hash_node3_value + "[PTR_L]" + ptr_left_child + "[PTR_R]" + ptr_right_child + "[FILE_LENGTH]" + str(len(node3_value))
            #print ('root_value', root_value)
            node3_encrypt = self.one_step_encrypt(key2_sym_priv, key2_mac_priv, root_value, "<DATA>")
            self.storage_server.put(ptr_node3, node3_encrypt)

            self.createMerkelChild(ptr_left_child, key2_sym_priv, key2_mac_priv, left_child_value)
            self.createMerkelChild(ptr_right_child, key2_sym_priv, key2_mac_priv, right_child_value)
    ...
    def createMerkelChild(self, ptr, key2_sym_priv, key2_mac_priv, value):
        ...
        ptr_left = self.crypto.get_random_bytes(64)
        ptr_right = self.crypto.get_random_bytes(64)
        node_value = "[HASH]" + hash_value + "[PTR_L]" + ptr_left + "[PTR_R]" + ptr_right

        node_encrypt = self.one_step_encrypt(key2_sym_priv, key2_mac_priv, node_value, "<CHILD>")
        self.storage_server.put(ptr, node_encrypt)
        '''
        print("size of hash " , len(bytes(hash_value,'utf-8')))
        print("size of ptr_left " , len(bytes(ptr_left,'utf-8')))
        print("size of node_value " , len(bytes(node_value,'utf-8')))
        print("size of node_encrypt " , len(bytes(node_encrypt,'utf-8')))
        '''
        if mid <= BLOCK_SIZE:
            ...
            self.createMerkelChild(ptr_left, key2_sym_priv, key2_mac_priv, left_child_value)
            self.createMerkelChild(ptr_right, key2_sym_priv, key2_mac_priv, right_child_value)
    ...
    def update(self, uid, sym_key, mac_key, new_value):
        ...
        if len(new_value) <= BLOCK_SIZE:
            ...
            #print('server put', len(new_value_encrypted))
            return
        try:
        ...
        pointer_left_child = result['[PTR_L]']
        pointer_right_child = result['[PTR_R]']
        #error catch

        self.efficientUpdate(pointer_left_child, sym_key, mac_key, left_child_value)
        self.efficientUpdate(pointer_right_child, sym_key, mac_key, right_child_value)


    def efficientUpdate(self, uid, sym_key, mac_key, new_value):
        ...
        try:
            ...
            if old_value_encrypted.startswith('<LEAF>'):
                ...
                ptr_left_child = results['[PTR_L]']
                ptr_right_child = results['[PTR_R]']
                #error catch
                updated_node_value = "[HASH]" + new_value_hash + "[PTR_L]" + ptr_left_child + "[PTR_R]" + ptr_right_child
                updated_node_encrypted = self.one_step_encrypt(sym_key, mac_key, updated_node_value, "<CHILD>")
                self.storage_server.put(uid, updated_node_encrypted)

                self.efficientUpdate(ptr_left_child, sym_key, mac_key, left_child_value)
                self.efficientUpdate(ptr_right_child, sym_key, mac_key, right_child_value)
        except:
            raise IntegrityError
    ...
    def node_decrypt(self, sym_key, mac_key, encrypted_value, header):
        ...
        if header.lower() == 'r':
            if msg_layer_2.startswith("[HASH]"):
                ...
                ptr_left_index = msg_layer_2.find("[PTR_L]")
                ptr_right_index = msg_layer_2.find("[PTR_R]")
                file_length_index = msg_layer_2.find("[FILE_LENGTH]")

                hash_val = msg_layer_2[hash_index+6:ptr_left_index]
                ptr_left = msg_layer_2[ptr_left_index+7:ptr_right_index]
                ptr_right = msg_layer_2[ptr_right_index+7: file_length_index]
                file_length = msg_layer_2[file_length_index+13:]
            ...
            hash_val = msg_layer_2[hash_index+6:ptr_left_index]
            ptr_left = msg_layer_2[ptr_left_index+7:ptr_right_index]
            ptr_right = msg_layer_2[ptr_right_index+7:]
    ...
    def download(self, name):
        try:
            ...
            pointer_right_child = results['[PTR_R]']
            pointer_left_child = results['[PTR_L]']
